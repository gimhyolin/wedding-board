...
function defaultForm(guest?: Guest | null): FormState {
  if (guest) {
    return {
      ...
      gift: String(guest.gift), envelopeNumber: guest.envelopeNumber, note: guest.note ?? '', isOnline: guest.isOnline ?? false,
    };
  }
  return {
    name: '', phone: '', side: 'groom', group: '친구', groupCustom: '',
    amount: '', mealTickets: '1', gift: '1',
    envelopeNumber: store.getNextEnvelopeNumber(), note: '', isOnline: false,
  };
}
        ...
        {!submitted && (dupConfirmed || !dupWarning) && (
          <>
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3.5 overflow-y-auto">
              ...
              {/* 축의금 */}
              <div>
                <label className="block text-[10px] font-semibold text-muted-foreground mb-1">축의금 *</label>
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-muted-foreground">계좌이체로 받은 축의금</p>
                </div>
                {form.isOnline && (
                  <span className="ml-auto text-[10px] font-semibold text-primary bg-white px-2 py-0.5 rounded-lg">온라인</span>
                )}
              </button>
