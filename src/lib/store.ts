...
export interface Guest {
  ...
  amountConfirmed: boolean;
  amountConfirmed: boolean; // 축의금 정산 완료 여부
}
...
export interface EventConfig {
  ...
  groomName: string;
  brideName: string;
  groomAccount: string; // 신랑 계좌
  brideAccount: string; // 신부 계좌
}
...
function guestToRow(g: Omit<Guest, 'id' | 'receivedAt' | 'thanksSent'>) {
  return {
    name:            g.name,
    phone:           g.phone,
    side:            g.side,
    group:           g.group,
    group_custom:    g.groupCustom ?? '',
    amount:          g.amount,
    meal_tickets:    g.mealTickets,
    gift:            g.gift,
    envelope_number: g.envelopeNumber,
    note:            g.note,
    is_online:        g.isOnline,
    amount_confirmed: g.amountConfirmed ?? false,
...
class WeddingStore {
  ...
  async fetchGuests() {
    ...
    if (error) { console.error('fetchGuests error:', error); return; }
  ...
  async fetchEventConfig() {
    const { data, error } = await supabase
      ...
      .eq('id', 1)
      .single();
    if (error) { console.error('fetchEventConfig error:', error); return; }
    if (data) {
      this.eventConfig = {
  ...
  async addGuest(data: Omit<Guest, 'id' | 'receivedAt' | 'thanksSent'>): Promise<Guest | null> {
    const { data: row, error } = await supabase
      .from('guests')
      .insert(guestToRow(data))
      .select()
      .single();
    if (error) { console.error('addGuest error:', error); return null; }
  ...
  async updateGuest(id: string, updates: Partial<Guest>) {
    ...
    const { error } = await supabase.from('guests').update(dbUpdates).eq('id', id);
    if (error) { console.error('updateGuest error:', error); return; }
    const idx = this.guests.findIndex(g => g.id === id);
    if (idx !== -1) {
      this.guests[idx] = { ...this.guests[idx], ...updates };
      this.notify();
    }
  }
  ...
  async deleteGuest(id: string) {
    ...
    if (error) { console.error('deleteGuest error:', error); return; }
  ...
  async deleteAllGuests() {
    ...
    if (error) { console.error('deleteAllGuests error:', error); return; }
    this.guests = [];
    this.notify();
  }
  ...
  async updateEventConfig(updates: Partial<EventConfig>) {
    ...
    const { error } = await supabase.from('event_config').update(dbUpdates).eq('id', 1);
    if (error) { console.error('updateEventConfig error:', error); return; }
  ...
  async updateConfig(updates: Partial<SettlementConfig>) {
    ...
    const { error } = await supabase.from('event_config').update(dbUpdates).eq('id', 1);
    if (error) { console.error('updateConfig error:', error); return; }
  ...
  getSettlement() {
    ...
    const ticketPct  = evt.totalMealTickets > 0 ? Math.min(Math.round((totalTickets / evt.totalMealTickets) * 100), 100) : 0;
    const guestPct   = evt.expectedGuests > 0 ? Math.min(Math.round((all.length / evt.expectedGuests) * 100), 100) : 0;
